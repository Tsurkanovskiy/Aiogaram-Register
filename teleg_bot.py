	...
	async with aiohttp.ClientSession() as session:
		async with session.post(CHECK_URL, data=str(message.text)) as response:
			...
			if resp['id_status'] == True:
				...
				await message.answer("Введіть свій пароль щоб зареєструвати користувача:")
			else:
...
async def check_user_id(message: types.Message, state: FSMContext):	
	...
	await RegisterStates.start.set()
	await message.answer(f"Вітаю! Вас зареєсторовано в системі!\nМожете зайти в свій профіль за даним посиланням:\n{DJ_URL}/login /")
